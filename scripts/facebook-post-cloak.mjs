...
async function autoLogin(cred) {
  ...
  await page.fill(emailSel, cred.account);

  // Fill password
  const passSel = 'input[name="pass"], input[type="password"]';
  await page.fill(passSel, cred.payload.password);
  await shot('autologin-filled');

  // Submit and wait for navigation
  const submitSel = 'button[name="login"], button[type="submit"], button:has-text("Log in")';
  await Promise.all([
    page.waitForNavigation({ timeout: 30000 }).catch(() => null),
    page.click(submitSel, { timeout: 10000 }).catch(() => page.press(passSel, 'Enter')),
  ]);
