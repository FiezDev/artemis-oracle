...
`nova-topic-relatedness-checker` returned `{suggestedFormat:"long_story", rationale:"transcript too short to segment confidently", confidence:0.35}` for **both** videos, including the 48k-char transcript. The transcript isn't reaching `ctx.input.transcript` (the upstream `[{transcript,…}]` shape isn't mapped to the `transcript` field the producer reads), so it always hits the `< 30 tokens` fallback and defaults to `long_story`. The gate's suggested format is therefore meaningless.
